...
extern cudaStream_t global_torch_cuda_stream;
...
void SpMMCsr(
  ...
  // sometimes dgl kernel not use torch cuda stream
  // so, we should set it manully.
  auto* thr_entry = runtime::CUDAThreadEntry::ThreadLocal();
  if(thr_entry->stream != global_torch_cuda_stream)
    thr_entry->stream = global_torch_cuda_stream;
