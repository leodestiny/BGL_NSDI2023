...
NDArray IndexSelect2D(NDArray array, IdArray index) {
  ...
  ATEN_XPU_SWITCH_CUDA(array->ctx.device_type, XPU, "IndexSelect", {
    ATEN_DTYPE_SWITCH(array->dtype, DType, "values", {
      ATEN_ID_TYPE_SWITCH(index->dtype, IdType, {
        ret = impl::IndexSelect2D<XPU, DType, IdType>(array, index);
      });
    });
  });
  return ret;
...
void IndexSelect2DAndScatter2D_(NDArray in, IdArray in_index, NDArray out, IdArray out_index) {
  ...
  //CHECK_GE(array->ndim, 1) << "Only support array with at least 1 dimension";
  //CHECK_EQ(index->ndim, 1) << "Index array must be an 1D array.";
  ATEN_XPU_SWITCH_CUDA(in->ctx.device_type, XPU, "IndexSelect", {
    ATEN_DTYPE_SWITCH(in->dtype, DType, "values", {
      ATEN_ID_TYPE_SWITCH(in_index->dtype, IdType, {
        impl::IndexSelect2DAndScatter2D_<XPU, DType, IdType>(in, in_index, out, out_index);
      });
    });
  });
}
...
void IndexSelect2DAndScatter2D_(NDArray in, IdArray in_index, NDArray out) {
  ...
  ATEN_XPU_SWITCH_CUDA(in->ctx.device_type, XPU, "IndexSelect", {
    ATEN_DTYPE_SWITCH(in->dtype, DType, "values", {
      ATEN_ID_TYPE_SWITCH(in_index->dtype, IdType, {
        impl::IndexSelect2DAndScatter2D_<XPU, DType, IdType>(in, in_index, out);
      });
    });
  });
}
...
void Scatter2D_(IdArray index, NDArray value, NDArray out) {
  ...
  ATEN_XPU_SWITCH_CUDA(value->ctx.device_type, XPU, "Scatter_", {
    ATEN_DTYPE_SWITCH(value->dtype, DType, "values", {
      ATEN_ID_TYPE_SWITCH(index->dtype, IdType, {
        impl::Scatter2D_<XPU, DType, IdType>(index, value, out);
      });
    });
  });
}
