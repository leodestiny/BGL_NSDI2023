...
class UnitGraph::CSR : public BaseHeteroGraph {
 public:
  ...
  CSR CopyTo(NDArray indptr, NDArray indices, NDArray data) const {
    if (Context() == indptr->ctx) {
      return *this;
    } else {
      return CSR(meta_graph_, adj_.CopyTo(indptr, indices, data));
    }
  }
...
HeteroGraphPtr UnitGraph::CopyTo(HeteroGraphPtr g, 
                            std::vector<NDArray>& target_indptr, 
                            std::vector<NDArray>& target_indices, 
                            std::vector<NDArray>& target_data) {
  if (target_indptr.size() == 0) {
    return g;
  }
  if (target_indptr[0]->ctx == g->Context()) {
    return g;
  } else {
    auto bg = std::dynamic_pointer_cast<UnitGraph>(g);
    CHECK_NOTNULL(bg);
    CSRPtr new_incsr =
      (bg->in_csr_->defined())? CSRPtr(new CSR(bg->in_csr_->CopyTo(target_indptr[0], target_indices[0], target_data[0]))) : nullptr;
    CSRPtr new_outcsr =
      (bg->out_csr_->defined())? CSRPtr(new CSR(bg->out_csr_->CopyTo(target_indptr[1], target_indices[1], target_data[1]))) : nullptr;
...
HeteroGraphPtr UnitGraph::CreateHeterographFrom(
    ...
    bool has_coo,
    dgl_format_code_t formats) {
  auto mg = CreateUnitGraphMetaGraph2();
