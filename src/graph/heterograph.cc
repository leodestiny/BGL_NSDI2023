...
// copy heterograph on allocated memory
HeteroGraphPtr HeteroGraph::CopyTo(HeteroGraphPtr g, 
  ...
  std::vector<HeteroGraphPtr> rel_graphs;
  for (auto g : hgindex->relation_graphs_) {
    rel_graphs.push_back(UnitGraph::CopyTo(g, target_indptr, target_indices, target_data));
  }
  //std::cout << " HeteroGraph rel_graphs layer:" << rel_graphs.size() << "\n";
  return HeteroGraphPtr(new HeteroGraph(hgindex->meta_graph_, rel_graphs,
                                        hgindex->num_verts_per_type_));
}
