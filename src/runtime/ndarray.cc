  ...
  static void DefaultDeleter(NDArray::Container* ptr) {
    ...
    if (ptr->manager_ctx != nullptr) {
      ...
      if(ptr->dl_tensor.ctx.device_type == 2)
	      //LOG(INFO) << "on delete " << ptr->dl_tensor.data  << " shape " << ptr->shape_[0] << std::endl;
	      //std::cout << "on delete " << ptr->dl_tensor.data  << " shape " << ptr->shape_[0] << std::endl;
...
NDArray NDArray::EmptySharedGPU(std::vector<int64_t> shape,
  ...
  // use ptr created by other process and shared by cudaIpcMemHandle 
