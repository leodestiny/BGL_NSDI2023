...
class NDArray {
 public:
  ...
  /*!
   * \brief Copy data content into another array using a specific stream
   * \param other The source array to be copied from.
   * \param stream The stream exeuted copy
   * \note The copy may happen asynchrously if it involves a GPU context.
   *       DGLSynchronize is necessary.
   */
  ...
  DGL_DLL static NDArray EmptyShared(const std::string &name,
                                     ...
                                     bool is_create);
  /*!
   * \brief Create an empty NDArray with CUDA IPC Memory.
   * \param shape The shape of the new array.
   * \param dtype The data type of the new array.
   * \param ctx The context of the Array.
   * \param ptr The pointer created by cudaIpcOpenMemHandle
   * \return The created Array
   */
