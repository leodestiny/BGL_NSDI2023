...
GET_PART_DATA = 901241

class GetPartDataResponse(rpc.Response):
    """Send full the partitioned data  back to client.
...
class GetPartDataRequest(rpc.Request):
    """Send data name to get the partitioned data shape from server.
    ...
    def process_request(self, server_state):
        ...
        if self.name not in kv_store.data_store:
            raise RuntimeError("KVServer Cannot find data tensor with name: %s" % self.name)
        data = kv_store.data_store[self.name]
        res = GetPartDataResponse(data)
        return res
...
class KVServer(object):
    ...
    def __init__(self, server_id, ip_config, num_servers, num_clients):
        ...
        rpc.register_service(DELETE_DATA,
                             DeleteDataRequest,
                             DeleteDataResponse)
        rpc.register_service(GET_PART_DATA,
                             GetPartDataRequest,
                             GetPartDataResponse)
