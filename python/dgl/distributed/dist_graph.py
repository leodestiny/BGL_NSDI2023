...
from .graph_partition_book import PartitionPolicy, get_shared_mem_partition_book, NodePartitionBook
...
def _copy_graph_to_shared_mem(g, graph_name, no_edge):
...
class DistGraphServer(KVServer):
    ...
    def __init__(self, server_id, ip_config, num_servers,
        ...
        if self.is_backup_server():
            ...
            self.client_g = None
            if isinstance(self.gpb, NodePartitionBook):
                self._no_edge = True
            else:
                self._no_edge = False
        else:
            ...
            #node_feats = {'feat': F.zeros((self.gpb._node_size,100), dtype=F.float32, ctx='cpu') }
            node_feats = {'feat': F.zeros((300000000,100), dtype=F.float32, ctx='cpu') }
            if isinstance(self.gpb, NodePartitionBook):
                self._no_edge = True
            else:
                self._no_edge = False
            ...
            if not disable_shared_mem:
                self.client_g = _copy_graph_to_shared_mem(self.client_g, graph_name, self._no_edge)
        ...
        if not self.is_backup_server():
            ...
            if not self._no_edge and edge_feats is not None:
...
class DistGraph:
    ...
    * When a user runs the training script with the distributed launch script, ``DistGraph`` will
    ...
    def __init__(self, graph_name, gpb=None, part_config=None):
        ...
        self._num_nodes = len(self._gpb._nid2partid)
        self._num_edges = 0
