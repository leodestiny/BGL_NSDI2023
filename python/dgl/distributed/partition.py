...
from .graph_partition_book import BasicPartitionBook, RangePartitionBook, NodePartitionBook
...
def load_partition_book(part_config, part_id, graph=None):
    ...
    # read tensor from hdfs when use a hdfs header
    # and convert to backend tensor
    if part_metadata['node_map'].startswith("hdfs://"):
        node_map = load_tensors(part_metadata['node_map'])['node_map']
    else:
        node_map = part_metadata['node_map'] if is_range_part else np.load(part_metadata['node_map'])
    if part_metadata['edge_map'] != '':
    ...
    if is_range_part:
        ...
        return NodePartitionBook(part_id, num_parts, node_map, graph), part_metadata['graph_name']
    else:
