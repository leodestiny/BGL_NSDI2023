...
def get_shared_mem_partition_book(graph_name, graph_part):
    ...
    if edge_map is None:
        return NodePartitionBook(part_id, num_parts, node_map, graph_part)
...
class GraphPartitionBook:
    ...
    * the local IDs of nodes and edges in a partition.
    ...
    def partid(self):
        ...
        """
class NodePartitionBook(GraphPartitionBook):
    """ partiton book only with node_map
    ...
    """
    def __init__(self, part_id, num_parts, node_map, part_graph):
        ...
        # nid_count contains the number of nodes in  -1,0,1,2,3, ..., partition
        ...
        # nid_count[0] contains the number of nodes in partition -1, which means no parition
        # these node should not contain in _partid2nids
        start = nid_count[0]
        for offset in nid_count[1:]:
    ...
    def shared_memory(self, graph_name):
        ...
        """
        # we use num_eges = 0 to indicate it is a NodePartitonBook
        self._meta, self._nid2partid, _ = _move_metadata_to_shared_mem(
            graph_name, self._num_nodes(), 0 , self._part_id, self._num_partitions,
            self._nid2partid, None, False)
