    ...
    for epoch in range(args.num_epochs):
        ...
        for p_id in range(args.num_parts):
            ...
            for step  in range(args.steps_per_partition):
                ...
                queue_step = step
                while True:
                    idx = queue_step % args.socket_num
                    try:
                        res = args.recv_queue[idx].get(timeout=0.01)
                        break
                    except:
                        queue_step += 1
                        continue
                ...
                if step % args.log_every == 0 and args.local_rank == 0:
                    ...
                    print('Avg sampling time {:.4f}s | Feature time {:.4f}s | Compute time {:.4f}s.'.format(
                        np.mean(iter_sampling_time[-5*20:]), np.mean(iter_feature_time[-5*20:]), np.mean(iter_compute_time[-5*20:])))
...
def main(args):
    ...
    if args.num_gpus > 1:
        ...
        print('rank:',th.distributed.get_rank(), ", world_size:", args.world_size)
    ...
    port = str(20003 + th.distributed.get_rank())
    #port = 20003
