...
class NeighborSampler(object):
    ...
    def sample_blocks(self, seeds):
        ...
        print("get tensor: {:.4f} s with input_node length {:d}".format(time.time() - start, len(input_nodes)))
...
def run(args, device, data):
    ...
    #        dev_id = 0
    #        model = th.nn.parallel.DistributedDataParallel(model, device_ids=[dev_id], output_device=dev_id,find_unused_parameters=True)
