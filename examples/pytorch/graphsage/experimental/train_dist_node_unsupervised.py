...
def run(args, device, data):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (pos_graph, neg_graph, blocks) in enumerate(dataloader):
            ...
            print("sample time {:.4f}".format(tic_step - start))
...
def main(args):
    ...
    train_eids = th.LongTensor(np.arange(g.number_of_edges()))
    train_nids = th.LongTensor(np.arange(g.number_of_nodes()))
    ...
    #train_nids = th.nonzero(train_mask, as_tuple=True)[0].long()
